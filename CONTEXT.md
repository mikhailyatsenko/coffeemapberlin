...
A map of Berlin's specialty coffee places where people can find, rate and review them.

## Language

**Place**:
A venue listed on the map. Coffee shops are the typical kind, but not the only one.
...
**Guest identity**:
The proof held in one browser that a Guest's Reviews and Favorites belong to them. Losing it leaves the Reviews public but no longer editable by that Guest.
...
**Rating**:
The numeric score inside a Review.
...
**Filters**:
Narrowing the Places shown by Neighborhood, minimum Average rating and Amenities.
_Avoid_: Tags filter, facets
...
**Journal**:
The site's section of editorial writing about coffee in Berlin.
_Avoid_: Blog, magazine, news

**Article**:
One piece of writing in the Journal.
