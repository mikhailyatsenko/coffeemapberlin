...
- [x] The document is written with the `writing-for-agents` skill, in English and imperative mood, one rule per bullet.
- [x] It is at most 120 lines.
- [x] The sections appear in this order: Scope, Enforced by ESLint, FSD, SOLID heuristics, Boy-scout and debt, Review.
- [x] "Enforced by ESLint" is one line, and the document doesn't restate the linted rules.
- [x] The FSD section covers:
  - layer purposes;
...
- [x] There is no whole-store heuristic, because lint covers it.
- [x] The Boy-scout and debt section covers:
...
- [x] The Review section holds the three `code-review` rules.
- [x] There are at most two code examples, each 6 lines or fewer: named re-export vs `export *`, and flag props vs `variant`/composition.
- [x] There are no references to files in `src/`.
- [x] `CLAUDE.md` has the `@` import, and `.cursorrules` is deleted.
